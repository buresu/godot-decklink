...
void DeckLinkOutput::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_texture", "texture"),
                       &DeckLinkOutput::set_texture);
  ClassDB::bind_method(D_METHOD("is_enabled"), &DeckLinkOutput::is_enabled);
  ClassDB::bind_method(D_METHOD("set_enabled", "enabled"),
                       &DeckLinkOutput::set_enabled);
  ClassDB::bind_method(D_METHOD("get_device"), &DeckLinkOutput::get_device);
  ClassDB::bind_method(D_METHOD("set_device", "device"),
                       &DeckLinkOutput::set_device);
  ClassDB::bind_method(D_METHOD("get_display_mode"),
                       &DeckLinkOutput::get_display_mode);
  ClassDB::bind_method(D_METHOD("set_display_mode", "display_mode"),
                       &DeckLinkOutput::set_display_mode);
  ClassDB::bind_method(D_METHOD("get_connection"),
                       &DeckLinkOutput::get_connection);
  ClassDB::bind_method(D_METHOD("set_connection", "connection"),
                       &DeckLinkOutput::set_connection);
  ClassDB::bind_method(D_METHOD("get_output_format"),
                       &DeckLinkOutput::get_output_format);
  ClassDB::bind_method(D_METHOD("set_output_format", "output_format"),
                       &DeckLinkOutput::set_output_format);
  ...
  ClassDB::add_property("DeckLinkOutput", {Variant::BOOL, "enabled"},
  ...
  ClassDB::add_property("DeckLinkOutput",
                        {Variant::INT, "connection", PROPERTY_HINT_ENUM,
                         "Unspecified:0,SDI:1,HDMI:2,Optical "
                         "SDI:4,Component:8,Composite:16,S-Video:32"},
...
ULONG DeckLinkOutput::AddRef() { return _ref_count.refval(); }

ULONG DeckLinkOutput::Release() { return _ref_count.unrefval(); }
...
bool DeckLinkOutput::open(int p_device, int64_t p_display_mode) {
  ...
  if (_output_format == OUTPUT_FORMAT_AUTO ||
      _output_format == OUTPUT_FORMAT_BGRA) {
    _output->DoesSupportVideoMode(_connection, _display_mode, bmdFormat8BitBGRA,
                                  bmdNoVideoOutputConversion,
                                  bmdSupportedVideoModeDefault, &actual_mode,
                                  &supported_bgra);
  }
  if (_output_format == OUTPUT_FORMAT_AUTO ||
      _output_format == OUTPUT_FORMAT_YUV || !supported_bgra) {
    _output->DoesSupportVideoMode(_connection, _display_mode, bmdFormat8BitYUV,
                                  bmdNoVideoOutputConversion,
                                  bmdSupportedVideoModeDefault, &actual_mode,
                                  &supported_yuv);
  }
  ...
  if (!supported_bgra && !supported_yuv) {
    UtilityFunctions::printerr("[DeckLinkOutput] Display mode is not supported "
                               "for selected output format.");
    close();
    return false;
  }
  ...
  HRESULT result =
      _output->EnableVideoOutput(_display_mode, bmdVideoOutputFlagDefault);
  if (result != S_OK) {
...
bool DeckLinkOutput::is_open() const { return _open; }

int DeckLinkOutput::get_width() const { return _width; }

int DeckLinkOutput::get_height() const { return _height; }
...
bool DeckLinkOutput::_output_frame(const PackedByteArray &p_rgba) {
  ...
  const int row_bytes =
      _pixel_format == bmdFormat8BitBGRA ? _width * 4 : _width * 2;

  IDeckLinkMutableVideoFrame *frame = nullptr;
  HRESULT result = _output->CreateVideoFrame(
      _width, _height, row_bytes, _pixel_format, bmdFrameFlagDefault, &frame);
  if (result != S_OK || !frame) {
    UtilityFunctions::printerr("[DeckLinkOutput] CreateVideoFrame failed: ",
                               (int64_t)result);
    return false;
  }

  IDeckLinkVideoBuffer *buffer = nullptr;
  if (frame->QueryInterface(IID_IDeckLinkVideoBuffer, (void **)&buffer) !=
          S_OK ||
      !buffer) {
    frame->Release();
    return false;
  ...
  if (access_started && buffer->GetBytes(&frame_bytes) == S_OK && frame_bytes) {
    ...
    if (_pixel_format == bmdFormat8BitBGRA) {
      ok = libyuv::ABGRToARGB(src, _width * 4, dst, row_bytes, _width,
                              _height) == 0;
    } else {
      PackedByteArray argb;
      argb.resize(_width * _height * 4);
      uint8_t *argb_bytes = argb.ptrw();
      ok = libyuv::ABGRToARGB(src, _width * 4, argb_bytes, _width * 4, _width,
                              _height) == 0 &&
           libyuv::ARGBToUYVY(argb_bytes, _width * 4, dst, row_bytes, _width,
                              _height) == 0;
    }
    if (!ok) {
      UtilityFunctions::printerr(
          "[DeckLinkOutput] libyuv conversion failed: pixel_format=",
          (int64_t)_pixel_format);
    } else {
      result = _output->DisplayVideoFrameSync(frame);
      ok = result == S_OK;
      if (!ok) {
        UtilityFunctions::printerr(
            "[DeckLinkOutput] DisplayVideoFrameSync failed: ",
...
Ref<Texture2D> DeckLinkOutput::get_texture() const { return _texture; }

void DeckLinkOutput::set_texture(Ref<Texture2D> p_texture) {
  _texture = p_texture;
}

bool DeckLinkOutput::is_enabled() const { return _enabled; }
...
int DeckLinkOutput::get_device() const { return _device; }
...
int64_t DeckLinkOutput::get_connection() const { return (int64_t)_connection; }
...
void DeckLinkOutput::_start_output_thread() {
  ...
  const Error error = _output_thread->start(
      callable_mp(this, &DeckLinkOutput::_output_thread_loop),
      Thread::PRIORITY_HIGH);
  if (error != OK) {
    UtilityFunctions::printerr(
        "[DeckLinkOutput] Could not start output thread: ", (int64_t)error);
...
HRESULT DeckLinkOutput::ScheduledFrameCompleted(
    IDeckLinkVideoFrame *p_completed_frame,
    BMDOutputFrameCompletionResult p_result) {
  return S_OK;
}
