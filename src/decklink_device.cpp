...
DeckLinkDevice::~DeckLinkDevice() { decklink::safe_release(_device); }

void DeckLinkDevice::_bind_methods() {
  ClassDB::bind_method(D_METHOD("get_model_name"),
                       &DeckLinkDevice::get_model_name);
  ClassDB::bind_method(D_METHOD("get_display_name"),
                       &DeckLinkDevice::get_display_name);
  ClassDB::bind_method(D_METHOD("to_dictionary"),
                       &DeckLinkDevice::to_dictionary);
  ClassDB::bind_method(D_METHOD("get_input_display_modes"),
                       &DeckLinkDevice::get_input_display_modes);
  ClassDB::bind_method(D_METHOD("get_output_display_modes"),
                       &DeckLinkDevice::get_output_display_modes);
}
...
String DeckLinkDevice::get_model_name() const { return _read_model_name(); }

String DeckLinkDevice::get_display_name() const { return _read_display_name(); }
...
IDeckLink *DeckLinkDevice::get_decklink() const { return _device; }

bool DeckLinkDevice::query_input(IDeckLinkInput **r_input) const {
  if (r_input) {
    *r_input = nullptr;
  }
  return _device && r_input &&
         _device->QueryInterface(IID_IDeckLinkInput, (void **)r_input) ==
             S_OK &&
         *r_input;
}
...
bool DeckLinkDevice::query_output(IDeckLinkOutput **r_output) const {
  ...
  return _device && r_output &&
         _device->QueryInterface(IID_IDeckLinkOutput, (void **)r_output) ==
             S_OK &&
         *r_output;
}

bool DeckLinkDevice::query_attributes(
    IDeckLinkProfileAttributes **r_attributes) const {
  if (r_attributes) {
    *r_attributes = nullptr;
  }
  return _device && r_attributes &&
         _device->QueryInterface(IID_IDeckLinkProfileAttributes,
                                 (void **)r_attributes) == S_OK &&
         *r_attributes;
}
